...
function datepicker() {
  $('.date-picker').each(function () {
    ...
    $input.datepicker({
      ...
      dateFormat: $input.data('format') ? $input.data('format') : 'yy-mm-dd'
    });
  });
}
...
/****  CKE Editor  ****/
function rteField() {
  if ($('.rte-field').length && $.fn.ckeditor) {

    $('.rte-field').each(function () {
      $(this).ckeditor();
    });
...
$(document).ready(function () {
  ...
  var checkboxes = $('body').find('.toggle-row-trigger');
  $.each(checkboxes, function (index, el) {
    var iCheckbox = $(this);
    var parentRow = iCheckbox.closest('.toggle-row');
    if (parentRow.get(0)) {
      parentRow.find('.iCheck-helper').on('click', function () {


        if (iCheckbox.is(':checked')) {

          parentRow.addClass('opacity-70').find('input[type="text"],select').attr('disabled', 'disabled');

        } else {

          parentRow.removeClass('opacity-70').find('input[type="text"],select').removeAttr('disabled');
  ...
  function incAttrName(name) {
    return name.replace(/^time_slots\[\d+\]*/g, function (f, form, number, label) {
      return form + (parseInt(number) + 1) + label;
    });
  }

  $('body').on('click', '.add-timeslot-row', function (e) {

    e.preventDefault();

    var timeSlotTbl = $('#branch-timeslot-tbl');
    var rowCount = 0;

    if (timeSlotTbl.get(0)) {
      rowCount = timeSlotTbl.find('tr.day-row').length;

      timeSlotTbl.find('tr.day-row').first().clone().find('input').each(function () {
